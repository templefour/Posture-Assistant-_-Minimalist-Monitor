export class UIController {
    ...
    if (!status) return;
    // 直接从 status 读取数值（兼容不同的字段名）
    const slouch = status.slouchValue !== undefined ? status.slouchValue : (status.metrics ? status.metrics.slouch : 0);
    const tilt = status.tiltAngle !== undefined ? status.tiltAngle : (status.metrics ? status.metrics.tilt : 0);
    ...
    if (this.slouchBar) {
        ...
        this.slouchBar.classList.toggle('bg-red-500', slouchPct > 100);
    }
